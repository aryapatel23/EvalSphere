...
export default function SignUp({ onSwitchToLogin, onSignUpSuccess }: SignUpProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || "Registration failed. Email might be in use.");
        }
      } catch (apiErr: any) {
        // Fallback to local mock database if backend is offline/unreachable
        const isNetworkError = !apiErr.status && (apiErr.message?.includes("fetch") || apiErr.message?.includes("NetworkError"));
        
        if (isNetworkError || apiErr.message === "Failed to fetch") {
          ...
          if (mockUsers.some((u: any) => u.email.toLowerCase() === email.toLowerCase())) {
            throw new Error("Email already in use. (Offline Mock Mode)");
          }
  ...
  return (
    <div className="w-full max-w-[440px] mx-auto px-4 py-8">
      ...
      {success && (
        <div className="mb-4 p-3 text-sm text-green-600 bg-green-50 border border-green-200 rounded-lg text-left">
      ...
      <form onSubmit={handleSubmit} className="space-y-5 text-left">
        ...
        {/* Role Notice */}
        <div className="flex items-center p-3 bg-violet-50 rounded-lg border border-violet-100 text-xs text-violet-700 font-sans">
          <svg
            className="w-4 h-4 mr-2 text-violet-600 flex-shrink-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
          ...
          You are registering as a <strong className="ml-1">Participant</strong>.
        </div>
        ...
        <button
          ...
          style={{
            backgroundColor: name && email && password && !loading && !success ? "#7c3aed" : "#c4c4c7",
          }}
        >
